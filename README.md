...
`tools/list` at `https://gateway.pipeworx.io/dogsapi/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/dogsapi_list_breeds \
  -H 'Content-Type: application/json' \
  -d '{"page":1}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/dogsapi_list_breeds`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
