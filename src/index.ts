...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'DogsAPI');
}
...
const tools: McpToolExport['tools'] = [
  {
    name: 'list_breeds',
    description: 'Search dog breeds with pagination. Returns breed names, IDs, weight ranges, life spans, and hypoallergenic status. Use get_breed for detailed info on a specific breed.',
    inputSchema: {
      type: 'object',
      properties: {
        page: {
  ...
  {
    name: 'get_breed',
    description: 'Get detailed info about a dog breed by ID. Returns characteristics, temperament, origin, size, and health data.',
    inputSchema: {
      type: 'object',
      properties: {
        id: {
  ...
  {
    name: 'list_facts',
    description: 'Get random dog facts. Returns interesting trivia about dog behavior, history, and abilities.',
    inputSchema: {
      type: 'object',
      properties: {
        limit: {
  ...
  {
    name: 'get_groups',
    description: 'Get all AKC dog breed groups (e.g., Sporting, Herding, Terrier). Returns group names and descriptions.',
    inputSchema: {
      type: 'object',
      properties: {},
    },
...
async function listBreeds(page: number) {
  const res = await pwFetch(`${BASE_URL}/breeds?page[number]=${page}`);
  if (!res.ok) throw await httpError(res, 'Dogs API error');
...
async function getBreed(id: string) {
  const res = await pwFetch(`${BASE_URL}/breeds/${encodeURIComponent(id)}`);
  if (!res.ok) throw await httpError(res, 'Dogs API error');
...
async function listFacts(limit: number) {
  const res = await pwFetch(`${BASE_URL}/facts?limit=${limit}`);
  if (!res.ok) throw await httpError(res, 'Dogs API error');
...
async function getGroups() {
  const res = await pwFetch(`${BASE_URL}/groups`);
  if (!res.ok) throw await httpError(res, 'Dogs API error');
